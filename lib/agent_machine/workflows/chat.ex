defmodule AgentMachine.Workflows.Chat do
  ...
  defp assistant_instructions do
    ...
    AgentMachine can route concrete tasks through direct, tool, planned, or swarm execution strategies inside the single agentic runtime. In planned strategy, a planner can delegate worker agents and the Elixir runtime starts them.
    In this direct strategy, you do not have tools, workers, or side effects.
    Do not claim that AgentMachine cannot use agents. If the user asks whether agents can be spawned, explain that concrete "use agents to do X" requests can use planned strategy, while this direct run cannot manually spawn arbitrary workers.
    Do not claim that you inspected files, used tools, changed state, or performed external side effects.
    If the request requires local files, tools, commands, or external side effects, say that this direct run cannot perform it.
    """
    |> String.trim()
  end
end
